...
try:
    ...
    if os.getenv("GROQ_API_KEY"):
        ...
        _MODEL = OpenAIResponses(id="gpt-4o")
    else:
...
def fmt_range(low, high):
    if low is None or high is None:
        return "N/A"
    return f"{fmt_price(low)}â€“{fmt_price(high)}"
...
def compute_quant_pack(symbol: str, period: str = "1y", rf_annual: float = 0.0, end_date: str = None) -> dict:
...
def backtest_walkforward(
    ...
    rebalance_freq: str = "ME",   # month-end (pandas â‰¥ 2.2)
