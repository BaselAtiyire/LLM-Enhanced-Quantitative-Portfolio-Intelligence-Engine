...
def compute_quant_pack(symbol: str, period: str = "1y", rf_annual: float = 0.0) -> dict:
    ...
    # 30D return + 30D annualized vol (based on last 31 closes)
    ret_30d = None
    vol_ann_30d = None
...
def get_stock_snapshot(symbol: str, period: str = "1y", rf_annual: float = 0.0) -> dict:
    ...
    return {
        ...
        # quant
        "ret_30d": qp["ret_30d"],            # decimal
        "vol_ann": qp["vol_ann"],            # decimal
        "sharpe": qp["sharpe"],
        "max_drawdown": qp["max_drawdown"],  # negative decimal
        "roll_vol_20d": qp["roll_vol_20d"],  # decimal
    }
...
def add_scores_and_ranks(rows: List[dict], weights: dict) -> List[dict]:
    ...
    metric_values = {m: [float(r[m]) for r in rows if r.get(m) is not None] for m in ws}
...
def build_table_markdown(rows: List[dict]) -> str:
    header = "| Rank | Ticker | Score | Price | Mkt Cap | 1W % | 30D Ret | Ann Vol | Sharpe | Max DD | Roll Vol(20D) | P/E | 52W Range |\n"
    sep = "| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |\n"
    lines = [header, sep]
    for r in rows:
        ...
        lines.append(
            f"| {r.get('rank','')} | {r['ticker']} | {score_str} | {fmt_price(r.get('price'))} | {fmt_money(r.get('market_cap'))} | "
            f"{fmt_pct_percent(r.get('one_week_pct'))} | {fmt_pct_decimal(r.get('ret_30d'))} | {fmt_vol(r.get('vol_ann'))} | "
            f"{fmt_num(r.get('sharpe'),2)} | {maxdd_str} | {fmt_vol(r.get('roll_vol_20d'))} | {fmt_num(r.get('trailing_pe'),2)} | "
...
def db_migrate(conn: sqlite3.Connection):
    ...
    Fixes: sqlite3.OperationalError: table snapshots has no column named sharpe
    """
    cur = conn.cursor()
...
def db_insert_run(conn: sqlite3.Connection, as_of: str, source: str, tickers: List[str], weights: dict) -> int:
    cur = conn.cursor()
    cur.execute(
...
def db_insert_snapshots(conn: sqlite3.Connection, run_id: int, rows: List[dict]):
    ...
    cur.executemany("""
        INSERT INTO snapshots (
            run_id, ticker, price, market_cap, one_week_pct, trailing_pe, week52_low, week52_high,
            ret_30d, vol_ann, sharpe, max_drawdown, roll_vol_20d, score, rank
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    """, [
        (
            run_id,
...
def backtest_walkforward(
    ...
    - Build daily returns for tickers.
    - Monthly rebalance:
        optimize max-Sharpe weights using prior lookback window (random Dirichlet sampling, long-only).
    ...
    if len(rets) < lookback_days + 30:
        return {"error": "Not enough data for the chosen lookback/period."}

    # month-end rebalance dates
    rebal_dates = rets.resample(rebalance_freq).apply(lambda x: x.index[-1]).dropna().values
    rebal_dates = [pd.Timestamp(d) for d in rebal_dates if pd.Timestamp(d) in rets.index]
    ...
    current_w_opt = np.ones(len(valid)) / len(valid)
    current_w_eqw = np.ones(len(valid)) / len(valid)
    ...
    def optimize_on_slice(ret_slice: pd.DataFrame) -> np.ndarray:
        ...
        if best_w is None:
            best_w = np.ones(len(valid)) / len(valid)
        return best_w
    ...
    eq_opt_s = pd.Series(eq_opt[1:], index=dates[1:], name="opt")
    eq_eqw_s = pd.Series(eq_eqw[1:], index=dates[1:], name="equal_weight")

    def summarize(equity: pd.Series):
        ...
        return {"ann_return": ar, "ann_vol": av, "sharpe": sh, "max_drawdown": mdd}
...
def run_chat_cli(scored_rows: List[dict], as_of: str, source: str):
    if _AGENT is None:
        print("\nAgno not available in this environment. Install agno + model deps.")
        return
...
def parse_cli(argv: List[str]):
    ...
    while i < len(argv):
        ...
        if a in {"--w_mcap", "--w_mom", "--w_ret", "--w_pe", "--w_vol"}:
            val = float(argv[i + 1])
            if a == "--w_mcap":
                weights["market_cap"] = val
            elif a == "--w_mom":
                weights["one_week_pct"] = val
            elif a == "--w_ret":
                weights["ret_30d"] = val
            elif a == "--w_pe":
                weights["trailing_pe"] = val
            elif a == "--w_vol":
                weights["vol_ann"] = val
            i += 2
            continue
...
if __name__ == "__main__":
    ...
    print(f"\nWeights used (normalized internally): {weights}")
    ...
    if flags["--store"]:
        ...
        db_init(conn)
        db_migrate(conn)
        run_id = db_insert_run(conn, as_of=as_of, source=source, tickers=tickers, weights=weights)
