...
# =============================
def get_close_history(symbol: str, period: str = "1y", end_date=None) -> pd.Series:
    t    = yf.Ticker(symbol)
...
def compute_quant_pack(symbol: str, period: str = "1y", rf_annual: float = 0.0, end_date=None) -> dict:
    closes = get_close_history(symbol, period=period, end_date=end_date)
...
# =============================
def get_stock_snapshot(symbol: str, period: str = "1y", rf_annual: float = 0.0, end_date=None) -> dict:
    t = yf.Ticker(symbol)
    try:
        info = t.info or {}
    except Exception:
    ...
    hist_7d      = t.history(period="7d", end=end_date)
    one_week_pct = None
    ...
    qp = compute_quant_pack(symbol, period=period, rf_annual=rf_annual, end_date=end_date)
